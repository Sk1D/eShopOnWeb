    ...
    public class CatalogItemListPagedEndpoint : IEndpoint<IResult, ListPagedCatalogItemRequest>
    {
        ...
        public async Task<IResult> HandleAsync(ListPagedCatalogItemRequest request)
        {
            try
            {
                ...
                _logger.LogInformation("INFO: Start get catalog items");
                ...
                {
                    response.PageCount =
                        int.Parse(Math.Ceiling((decimal)totalItems / request.PageSize.Value).ToString());
                }
                else
                {
                ...
                _logger.LogInformation($"INFO: Count catalog items:{response.CatalogItems?.Count}");             

                _logger.LogInformation("INFO: END get catalog items");
                return Results.Ok(response);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "it is a new Exception");
